...
Our work builds upon three key prior contributions. Ellen et al. \cite{ellen2010nonblocking} introduced a practical lock-free BST using fine-grained Compare-and-Swap (CAS) operations and a cooperative helping mechanism to resolve conflicts. While highly efficient for standard updates (\ins{}, \del{}, \contains{}), this structure provides no mechanism for aggregate queries. Figure~\ref{fig:ellen-operations} illustrates the lock-free insert and delete operations from Ellen et al.'s work, showing how structural modifications are performed using atomic CAS operations.

\begin{figure}[H]
\centering
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/elen_insert.png}
    \caption*{Insert operation: Adding key 5 to tree (4,2)}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/ellen_delete.png}
    \caption*{Delete operation: Removing key 5 from tree (2,4,5)}
\end{minipage}
\caption{Lock-free BST operations from Ellen et al. \cite{ellen2010nonblocking}}
\label{fig:ellen-operations}
\end{figure}

Fatourou and Ruppert \cite{fatourou2024lockfree} extended this foundation by augmenting each BST node with immutable \codestyle{Version} objects. Each Version forms a snapshot of the tree at a specific point in time, storing subtree metadata (\codestyle{nbChild}) that enables wait-free computation of \size{}, \rank{}, and \select{}. The key insight is that updates propagate new Version objects from modified nodes to the root, creating a consistent view for concurrent queries. However, this generality comes at a cost: \textit{every} update must allocate and CAS new Version nodes along a path to the root, even when no queries are running. Figure~\ref{fig:fatourou-propagation} demonstrates this propagation mechanism, showing how an insert operation creates new Version nodes along the path from the modified node to the root.

\begin{figure}[H]
\centering
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/propagate_context.png}
    \caption*{Before insert: Original Version tree}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/propagate_insert.png}
    \caption*{After insert: New Versions propagated to root}
\end{minipage}
\caption{Version propagation in Lock-Free Augmented BST \cite{fatourou2024lockfree}}
\label{fig:fatourou-propagation}
\end{figure}
...
Figure~\ref{fig:fast-slow-metadata} illustrates how fast metadata (\codestyle{fastSize}) and slow metadata (\codestyle{Version} with \codestyle{nbChild}) coexist after a fast operation. The fast path updates only the \codestyle{fastSize} counters, leaving the Version tree unchanged.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{images/fast_path.png}
\caption{Fast and slow metadata coexisting after a fast operation. Fast path updates \codestyle{fastSize} only, leaving Version tree stale (Red : fast metadata, Green : sum of slow and fast metadata).}
\label{fig:fast-slow-metadata}
\end{figure}
...
Figure~\ref{fig:forwarding-pointer} shows how forwarding pointers enable queries to navigate from stale Version nodes to current replacement nodes. When a node becomes orphaned during a fast operation, its \codestyle{forwardingPtr} redirects queries to the valid replacement in the updated tree structure.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{images/FP_exp.png}
\caption{Forwarding pointer mechanism: orphaned nodes redirect queries to replacement nodes}
\label{fig:forwarding-pointer}
\end{figure}
...
Figure~\ref{fig:chain-formation} illustrates how chains of forwarding pointers can form through successive fast operations. Starting from a normal tree state, an insert creates one forwarding pointer, and a subsequent delete creates another, resulting in a chain that queries would need to traverse.

\begin{figure}[H]
\centering
\begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/FP_context.png}
    \caption*{Initial tree state}
\end{minipage}
\hfill
\begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/FP_insert.png}
    \caption*{After fast insert: one forwarding pointer}
\end{minipage}
\hfill
\begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/FP_delete.png}
    \caption*{After fast delete: chain of forwarding pointers}
\end{minipage}
\caption{Formation of forwarding pointer chains through successive fast operations}
\label{fig:chain-formation}
\end{figure}
...
Figure~\ref{fig:reverse-pointers} demonstrates how reverse pointers enable chain compression. After the insert operation, a reverse pointer is established from the orphaned node to its successor. When the delete operation occurs, the reverse pointer allows the predecessor to bypass the intermediate node and point directly to the final target, maintaining a chain depth of at most 1.

\begin{figure}[H]
\centering
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/RP_insert.png}
    \caption*{After insert: reverse pointer established}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{images/RP_delete.png}
    \caption*{After insert and delete: chain compressed using reverse pointer}
\end{minipage}
\caption{Reverse pointers enabling constant-time chain compression}
\label{fig:reverse-pointers}
\end{figure}
